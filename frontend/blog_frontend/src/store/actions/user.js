...
// updated
export const userProjectEditInit = () => {
    return {
        type: actionTypes.USER_PROJECT_EDIT_INIT
    };
};

export const userProjectEditSuccess = () => {
    return {
        type: actionTypes.USER_PROJECT_EDIT_SUCCESS
    };
};

export const userProjectEditFail = error => {
    return {
        type: actionTypes.USER_PROJECT_EDIT_FAIL,
        error: error
    };
};

export const userProjectEdit = (updatedProject, config) => {
    return dispatch => {
        dispatch(userProjectEditInit());
        AxiosInstance.post("/dashboard/update-project/", updatedProject, config)
            .then(response => {
                alert("Project Updated Successfully");
                dispatch(userProjectEditSuccess());
            })
            .catch(error => {
                alert("Something Went Wrong... Try Again");
                dispatch(userProjectEditFail(error));
            });
    };
};
