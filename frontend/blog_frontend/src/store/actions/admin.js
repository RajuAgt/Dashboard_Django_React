...
// Updated
export const adminViewAllProjectsInit = () => {
    return {
        type: actionsTypes.ADMIN_VIEW_ALL_PROJECTS_INIT
    };
};

export const adminViewAllProjectsSuccess = projectsData => {
    return {
        type: actionsTypes.ADMIN_VIEW_ALL_PROJECTS_SUCCESS,
        projectsData: projectsData
    };
};

export const adminViewAllProjectsFail = error => {
    return {
        type: actionsTypes.ADMIN_VIEW_ALL_PROJECTS_FAIL,
        error: error
    };
};

export const adminViewAllProjects = config => {
    return dispatch => {
        dispatch(adminViewAllProjectsInit());
        AxiosInstance.get("/admin-panel/projects/", config)
            .then(response => {
                dispatch(adminViewAllProjectsSuccess(response.data));
            })
            .catch(error => {
                alert("ERROR..!! Something Went Wrong");
                dispatch(adminViewAllProjectsFail(error));
            });
    };
};

export const adminEditProjectInit = () => {
    return {
        type: actionsTypes.ADMIN_EDIT_PROJECT_INIT
    };
};

export const adminEditProjectSuccess = () => {
    return {
        type: actionsTypes.ADMIN_EDIT_PROJECT_SUCCESS
    };
};

export const adminEditProjectFail = error => {
    return {
        type: actionsTypes.ADMIN_EDIT_PROJECT_FAIL,
        error: error
    };
};

export const adminEditProject = (config, slug) => {
    return dispatch => {
        dispatch(adminEditProjectInit());
        AxiosInstance.patch(
            ...
            .then(response => {
                dispatch(adminEditProjectSuccess());
                alert("Project Edited Successfully");
            })
            .catch(error => {
                dispatch(adminEditProjectFail(error));
                alert("ERROR...!! Something Went Wrong");
                console.log(error.response.data);
            });
    };
